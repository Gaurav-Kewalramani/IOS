...
class OneTimeCodeViewController: UIViewController {
    ...
    @IBAction func resendCode(_ sender: Any) {
        self.toggleResendCodeOption(isEnabled: false)
                DispatchQueue.main.asyncAfter(deadline: .now() + 60) { [weak self] in
                    self?.toggleResendCodeOption(isEnabled: true)
                }
                self.authenticationViewModel.resendCode(withPhoneNumber:phoneNumber) { [weak self] (isResent, error) in
                    guard isResent else {
                        return
                    }
                }
...
extension OneTimeCodeViewController {
    ...
    func toggleResendCodeOption(isEnabled: Bool) {
        self.resendCodeButton.isEnabled = isEnabled
        self.resendCodeButton.alpha     = isEnabled ? 1.0 : 0.5
    }
